    ...
    dataset_open_(false)
{
    ...
    // build a lookup table of stream number to stream storage type
    //std::vector<Stream> streams = xmlHeader.streams;
    //std::map<int, int> stream_types;
    //for (std::vector<Stream>::const_iterator it = streams.begin(); it != streams.end(); ++it) {
        //stream_types[it->number] = it->dataType;
    //}
}
...
void Dataset::appendAcquisition(const Acquisition& acq, int stream_number)
{
    ...
    std::vector<std::complex<float> > data = acq.getData<std::complex<float> >();
...
Acquisition Dataset::readAcquisition(unsigned long index, int stream_number)
{
    ...
    // TODO: fix this ASAP:
    std::vector<std::complex<float> > tmp = acq.getData<std::complex<float> >();
    if (obj.data.len != tmp.size() * 2) {
