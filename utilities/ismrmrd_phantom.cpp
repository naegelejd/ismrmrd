...
boost::shared_ptr<NDArray<std::complex<float> > > phantom(std::vector<PhantomEllipse>& ellipses, unsigned int matrix_size)
{
...
boost::shared_ptr<NDArray<std::complex<float> > > shepp_logan_phantom(unsigned int matrix_size)
{
...
boost::shared_ptr<NDArray<std::complex<float> > > generate_birdcage_sensititivies(unsigned int matrix_size, unsigned int ncoils, float relative_radius)
{
    ...
    for (unsigned int c = 0; c < ncoils; c++) {
        ...
        for (unsigned int y = 0; y < matrix_size; y++) {
            float y_co = (1.0*y-(matrix_size / 2))/(matrix_size / 2)-coily;
            for (unsigned int x = 0; x < matrix_size; x++) {
...
int add_noise(NDArray<std::complex<float> > & a, float sd)
{
...
int add_noise(Acquisition& a, float sd)
{
    ...
    for (uint16_t c=0; c<a.getActiveChannels(); c++) {
        for (uint16_t s=0; s<a.getNumberOfSamples(); s++) {
            a.at<std::complex<float> >(s,c) += std::complex<float>(var_nor(), var_nor());
        }
    }
